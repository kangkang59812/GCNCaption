...
def train_scst(model, dataloader, optim, cider, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, *args in enumerate(dataloader):
            detections, selfbboxes, bboxes = args[0][0].to(
                device), args[0][1].to(device), args[0][2].to(device)
            outs, log_probs = model.beam_search(detections, selfbboxes, bboxes, seq_len, text_field.vocab.stoi['<eos>'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs_folder', type=str, default='debug')
    parser.add_argument('--save_folder', type=str, default='/home/lkk/code/meshed-memory-transformer/saved_models0-5')
    ...
    if args.resume_last or args.resume_best:
        if args.resume_last:
            fname = (args.save_folder+'/%s_last.pth') % args.exp_name
        else:
            fname = (args.save_folder+'/%s_best.pth') % args.exp_name
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if switch_to_rl and not best:
            data = torch.load((args.save_folder+'/%s_best.pth') % args.exp_name)
            torch.set_rng_state(data['torch_rng_state'])
            torch.cuda.set_rng_state(data['cuda_rng_state'])
            np.random.set_state(data['numpy_rng_state'])
            random.setstate(data['random_rng_state'])
        ...
        if best:
            copyfile((args.save_folder+'/%s_last.pth') % args.exp_name,
