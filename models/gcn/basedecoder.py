...
from .gcnutils import penalty_builder, split_tensors
...
class BaseDecoder(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def add_diversity(beam_seq_table, logprobs, t, divm, diversity_lambda, bdash):
            ...
            if divm > 0:
                ...
                if local_time == 0:
                    ...
                    logprobs = logprobs - \
                        self.repeat_tensor(bdash, change) * diversity_lambda
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            if t > 0:
                ...
                beam_seq = beam_seq.gather(
                    1, beam_ix.unsqueeze(-1).expand_as(beam_seq))
            ...
            beam_logprobs = unaug_logprobs.reshape(batch_size, -1, vocab_size).gather(
                1, beam_ix.unsqueeze(-1).expand(-1, -1, vocab_size))  # NxbxV
        ...
        # INITIALIZATIONS
        beam_seq_table = [torch.LongTensor(batch_size, bdash, 0).to(
            device) for _ in range(group_size)]
        beam_seq_logprobs_table = [torch.FloatTensor(
            batch_size, bdash, 0, self.vocab_size).to(device) for _ in range(group_size)]
        beam_logprobs_sum_table = [torch.zeros(
            batch_size, bdash).to(device) for _ in range(group_size)]

        # logprobs # logprobs predicted in last time step, shape (beam_size, vocab_size+1)
        done_beams_table = [
            [[] for __ in range(group_size)] for _ in range(batch_size)]
            ...
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if remove_bad_endings and t-divm > 0:
                        logprobs[torch.from_numpy(np.isin(beam_seq_table[divm][:, :, t-divm-1].cpu(
                        ).numpy(), self.bad_endings_ix)).reshape(-1), 0] = float('-inf')
                    # suppress UNK tokens in the decoding
                    if logprobs.size(1)-1 == suppress_UNK:
                        logprobs[:, logprobs.size(
                            1)-1] = logprobs[:, logprobs.size(1)-1] - 1000
        ...
        done_beams_table = [[sorted(done_beams_table[b][i], key=lambda x: -x['p'])[:bdash]
                             for i in range(group_size)] for b in range(batch_size)]
