...
class BeamSearch(object):
    ...
    def _expand_visual(self, visual: utils.TensorOrSequence, selfbboxes: utils.TensorOrSequence, bboxes: utils.TensorOrSequence, cur_beam_size: int, selected_beam: torch.Tensor):
        if isinstance(visual, torch.Tensor):
            ...
            selected_beam_red_size = (
                self.b_s, self.beam_size) + tuple(1 for _ in range(len(visual_exp_shape) - 2))
            selected_beam_exp_size = (
                self.b_s, self.beam_size) + visual_exp_shape[2:]
            ...
            selected_beam_red_size = (
                self.b_s, self.beam_size) + tuple(1 for _ in range(len(selfbboxes_exp_shape) - 2))
            selected_beam_exp_size = (
                self.b_s, self.beam_size) + selfbboxes_exp_shape[2:]
            selfbboxes_exp = selfbboxes.view(selfbboxes_exp_shape)
            selected_beam_exp = selected_beam.view(
                selected_beam_red_size).expand(selected_beam_exp_size)
            selfbboxes = torch.gather(
                selfbboxes_exp, 1, selected_beam_exp).view(selfbboxes_red_shape)

            bboxes_shape = bboxes.shape
            bboxes_exp_shape = (self.b_s, cur_beam_size) + bboxes_shape[1:]
            bboxes_red_shape = (self.b_s * self.beam_size,) + bboxes_shape[1:]
            selected_beam_red_size = (
                self.b_s, self.beam_size) + tuple(1 for _ in range(len(bboxes_exp_shape) - 2))
            selected_beam_exp_size = (
                self.b_s, self.beam_size) + bboxes_exp_shape[2:]
            bboxes_exp = bboxes.view(bboxes_exp_shape)
            selected_beam_exp = selected_beam.view(
                selected_beam_red_size).expand(selected_beam_exp_size)
            bboxes = torch.gather(
            ...
            for im in visual:
                ...
                visual_exp_shape = (self.b_s, cur_beam_size) + visual_shape[1:]
                visual_red_shape = (
                    self.b_s * self.beam_size,) + visual_shape[1:]
                selected_beam_red_size = (
                    self.b_s, self.beam_size) + tuple(1 for _ in range(len(visual_exp_shape) - 2))
                selected_beam_exp_size = (
                    self.b_s, self.beam_size) + visual_exp_shape[2:]
    ...
    def apply(self, visual: utils.TensorOrSequence, selfbboxes: utils.TensorOrSequence, bboxes: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        self.seq_mask = torch.ones(
            (self.b_s, self.beam_size, 1), device=self.device)
        ...
        seq_logprob, sort_idxs = torch.sort(
            self.seq_logprob, 1, descending=True)
        outputs = torch.cat(outputs, -1)
        outputs = torch.gather(outputs, 1, sort_idxs.expand(
            self.b_s, self.beam_size, self.max_len))
        log_probs = torch.cat(self.log_probs, -1)
        log_probs = torch.gather(log_probs, 1, sort_idxs.expand(
            self.b_s, self.beam_size, self.max_len))
        if return_probs:
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, selfbboxes: utils.TensorOrSequence, bboxes: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        ...
        if t > 0:
            mask = (self.selected_words.view(self.b_s, cur_beam_size)
                    != self.eos_idx).float().unsqueeze(-1)
            self.seq_mask = self.seq_mask * mask
            word_logprob = word_logprob * self.seq_mask.expand_as(word_logprob)
            old_seq_logprob = self.seq_logprob.expand_as(
                candidate_logprob).contiguous()
            old_seq_logprob[:, :, 1:] = -999
            candidate_logprob = self.seq_mask * candidate_logprob + \
                old_seq_logprob * (1 - self.seq_mask)

        selected_idx, selected_logprob = self.select(
            t, candidate_logprob, **kwargs)
        selected_beam = selected_idx / candidate_logprob.shape[-1]
        selected_words = selected_idx - \
            selected_beam * candidate_logprob.shape[-1]
        ...
        self.seq_mask = torch.gather(
            self.seq_mask, 1, selected_beam.unsqueeze(-1))
        outputs = list(torch.gather(o, 1, selected_beam.unsqueeze(-1))
                       for o in outputs)
        outputs.append(selected_words.unsqueeze(-1))

        if return_probs:
            if t == 0:
                self.all_log_probs.append(word_logprob.expand(
                    (self.b_s, self.beam_size, -1)).unsqueeze(2))
            else:
